...
const BilliardsAudio = (() => {
  ...
  /**
   * 玉が**空中を飛んでいる間**の「ぴょーん」。
   * 飛び立った時に鳴らし、飛んでいる時間ぶんだけ引き伸ばす。
   * 着地の音ではない（着地はクッションと同じ「ドン」）。
   *
   * ここだけは**音程が滑るのが正解**で、漫画の跳ねる音そのもの。
   * 玉どうしの衝突で同じことをすると電子音に聞こえるので、そちらは録音した素材のまま。
   *
   * 続けて跳ねたときは鳴らし直す。ただし前の音がまだ鳴っている間は小さめにして、
   * 重なって騒がしくならないようにする。
  ...
  function boing(strength, seconds) {
    ...
    const s = Math.max(0.15, Math.min(1, strength == null ? 0.6 : strength));
    const now = ctx.currentTime;
    const overlap = now < boingUntil;
    const dur = Math.max(0.18, Math.min(3.5, seconds || 0.3));
    boingUntil = now + dur;
    const lvl = (0.16 + 0.20 * s) * (overlap ? 0.4 : 1);
    const t = now + 0.004;

    // 音程が下へ揺れながら落ちる。長い跳躍ほどゆっくり落ちる
    const f0 = 640 * (0.85 + 0.4 * s);
    const N = 48, curve = new Float32Array(N);
    for (let i = 0; i < N; i++) {
      const k = i / (N - 1);
      const fall = f0 * Math.pow(0.30, k);                        // 下へ落ちる
      const wob = 1 + 0.14 * Math.sin(k * Math.PI * 6) * (1 - k); // 揺れは出だしほど大きい
      curve[i] = fall * wob;
    }
    ...
    const g = ctx.createGain();
    // 長く伸ばすので、出だしだけ立ち上げて、あとは緩やかに減らす
    g.gain.setValueAtTime(0, t);
    g.gain.linearRampToValueAtTime(lvl, t + 0.012);
    g.gain.exponentialRampToValueAtTime(Math.max(0.0001, lvl * 0.35), t + dur * 0.7);
  ...
  /**
   * ミッション達成の音（8.5.11節の演出）。**上りの分散和音（ファンファーレ）**。
   *
   * ★**COMBO と同じ一撞きで同時に鳴りうる**（反則の音と違い、連鎖と課題は打ち消し合わない）。
   *   そこで**音の形そのものを変えてある** ── COMBO は3音の和音を**同時に**積むが、
   *   こちらは4音を**順に駆け上がって最後の音を伸ばす。**重なっても2つ鳴ったと分かる。
   * ★**音程は滑らせない**（滑らせると合成音が電子音に聞こえる）。
   * ★**達成感は「終わりの音を伸ばすこと」と「上に薄く輝きを重ねること」で出す**（利用者指示）。
   *   最後のドだけ 0.5 秒伸ばし、5度と1オクターブ上を**ごく薄く**添え、
   *   低いドとソで下を支える。音量そのものは上げない（他の音より突出させない）。
   * ★連鎖と違って**高さは変えない。**達成は1回ごとに完結していて、積み上がる数が無い。
   */
  function missionSfx() {
    if (!audioOn || !ctx || !sfxGain) return;
    const t0 = ctx.currentTime + 0.005;
    const note = (f, at, dur, peak, type) => {
      const o = ctx.createOscillator();
      o.type = type || 'triangle';
      o.frequency.value = f;
      const g = ctx.createGain();
      env(g, t0 + at, 0.004, peak, dur);
      o.connect(g).connect(sfxGain);
      o.start(t0 + at); o.stop(t0 + at + dur + 0.03);
    };
    // ド・ミ・ソ・ド（C5→C6）を駆け上がる。最後のドだけ長く伸ばす
    note(523.25, 0, 0.17, 0.20);
    note(659.26, 0.075, 0.17, 0.21);
    note(783.99, 0.150, 0.19, 0.23);
    note(1046.50, 0.225, 0.52, 0.26);
    // 最後の音に重ねる輝き（5度と1オクターブ上）。薄く・短く
    note(1567.98, 0.245, 0.34, 0.06, 'sine');
    note(2093.00, 0.265, 0.26, 0.045, 'sine');
    // 下を支える低いドとソ（厚みだけを足す）
    note(261.63, 0.225, 0.46, 0.09, 'sine');
    note(392.00, 0.225, 0.46, 0.06, 'sine');
  }
   ...
   */
  function sfx(name, strength, seconds) {
    if (!audioOn || !ctx || !sfxGain) return;
    /*
