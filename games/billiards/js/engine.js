...
const BilliardsEngine = (() => {
  ...
  function createWorld(table, balls, tuning, opts) {
    return {
       ...
       * エンジンはこの中身を読まない。1刻みごとに BilliardsField.apply へ渡すだけで、
  ...
  function cloneWorld(w) {
    return {
       ...
       *   onAdvance は「本物の盤面を書き換える通知」なので複製へ持ち込むと読むたびに本物が汚れるが、
       *   ギミックは**盤面そのものの姿**なので、持ち込まないと
       *   AIだけが地震も傾きも無い盤面を読むことになる（11.5.3節＝ギミック盤面でもAIが手番を務める）。
  ...
  function step(w) {
    ...
    for (const b of live) {
       ...
       * リプレイ・AIの読み）ので、呼ぶ側に書き足すと必ずどれかが抜け、
       * その道を通ったぶんだけギミックの効かない盤面ができる。
       * 通常モードでは w.field が null なので、この行は素通りする（費用ゼロ）。
       *
