...
const BilliardsRules = (() => {
  ...
  const FOUL_TABLE = {
     ...
     *     手玉と的球の区別が無いので「手玉がポケットに落ちる＝スクラッチ」という概念が無い。
     *     投げた玉がポケットへ落ちたら、**持ち球を1個失う**だけ（盤から消えて数えない）で、
   ...
   */
  const CURLING_STONES = 4;      // 1人あたりの持ち球（D404で確定。7.9.1節）
  ...
  function endBanking(game, winner) {
    ...
    let first = winner;
    /*
     * ★カーリング型は**バンキングで勝った人を「最後に撞く人」にする**（利用者指示）。
     *
     * カーリングでは**エンドの最後の1投（ハンマー）を持つ側が有利**である
     * ── 相手の置いた形を見てから最後に決められるからで、現実の競技でも
     * ハンマーは「前のエンドで得点しなかった側」に回る優遇として扱われる。
     * したがって**バンキングの勝者は後手**にする。
     *
     * 席の巡りは「先頭が最初に撞く」形なので、**勝った人の次の席を先頭へ回す**と
     * 勝った人が巡りの末尾＝最後に撞く人になる。
     * エンドごとに先頭を1つ送る仕掛け（curlingCloseEnd）はそのまま働き、
     * **人数と同じエンド数なら全員がちょうど1回ずつハンマーを持つ。**
     */
    if (game.rule === 'G-09') {
      rotateSeats(game, nextSeat(game, winner));
      first = seatCycle(game)[0];
      if (!isActive(game.players[first])) first = firstActive(game);
    }
    /*
  ...
  function award(game, r, raw, opt) {
     ...
     * ★**0点の一撞きでは、倍率も立てない。**
     *
     *   ローテーションは**1球も落ちなくてもこの口を通る**（落とした玉の番号の合計＝0）。
     *   そこで倍率だけを結果へ残すと、**画面が「COMBO ×4!　+0」と出す**
     *   ── 実機で利用者が見つけた不具合そのものである。
     *   ★しかもその一撞きは**連鎖が途切れる側**（0点なので次から×1へ戻る。8.3.3節）なので、
     *   **成立していない倍率を知らせていた。**得点の計算は正しく、**表示だけが嘘をついていた。**
     *
     *   ★**ここで止めるのは、得点を足す口が1か所しかないからである。**
  ...
  function bowlRack(game, frame) {
    ...
    if (frame) frame.down = {};
    /*
     * ★**跡はここでは消さない**（利用者指示。いちど消す形にしたが戻した）。
     *
     * フレームの2投目は必ず「投げ終わり＝並べ直し」なので、ここで消すと
     * **2投目の結果だけ一度も見られない**（実機の指摘）。
     * 跡を消すのは**次に撞き始めたとき**（app.js の applyShot）1か所だけにする。
     * 重なって見えないよう、**跡には数字を書かない**（利用者指示）。
     */
  }
  ...
  function bowlAdvance(game) {
    ...
    /*
     * ★**10本ぜんぶの「倒れた／残った」を控える**（利用者指示）。
     *   画面には初期配置の形のボックスで出す。**moves は「この一投の前に立っていた玉」しか
     *   持たない**ので、前の投で倒れた玉が抜ける。ボックスは10本そろっていないと形が読めない。
     *   ★**並べ直す前に控える**（並べ直すと倒れた印が消える）。
     */
    const pinState = [];
    for (const b of game.world.balls) {
      if (b.pin == null) continue;
      pinState.push({ pin: b.pin, num: b.num, down: !!f.down[b.id] });
    }
    pinState.sort((a, b) => a.pin - b.pin);
    bw.pinState = pinState;
    bw.voidRoll = false;
    f.rolls.push(n);
    game.inningNo++;
   ...
   *   | 合計 |             | 24 | 27 |
   *
   * ★**この表は第52セッション（2026-09-07）のもので、そのあと2ホールが動いている。**
   *   いま効いている値は下の GOLF_PAR で、**L字 4**（第53）と**星型 3**（第62）に変わっている。
   *   合計は 27 → 26。**この表は当時の測定の記録として残してある。**
   *
   ...
   *
   * ★**十字（A-09）の「hard でも5回に1回上がれない」は第53セッションで解消した**
   *   （AIが手玉を台の外へ置く不具合だった）。
   * ★**星型（A-11）の順位逆転は第62セッション（2026-09-12）で解消した。**
   *   逆転の中身は「ぶれの入れ方」ではなく**上級者の6打のうち2打が罰打**で、
   *   その1つが毎回同じ池ポチャだった（大もとは AI が池をまん丸として読むこと）。
   *   **利用者判断で、星型のホールだけ池を半分にした**（table.js の GOLF_WATER_SCALE）。
   *   その結果、上級者は 6打 → **3打**（3周とも同じ）・中級者は中央値 8打で順序が戻った。
   *   **他の7ホールは1打も動いていない**（実測で確認：3/1/3/2/4/4/2 が前後で一致）。
   *
   *   測定の生データと読み取り＝`L:/momo/claude/billiards-checks/par_result_20260907.md`
   *   （星型の測り直し＝同じ場所の `star_result_20260912.md`）
   */
  const GOLF_PAR = {
    'A-01': 3, 'A-02': 3, 'A-04': 3, 'A-06': 3,
    'A-07': 4, 'A-08': 4, 'A-09': 3, 'A-11': 3,
  };
  /*
   * ★**L字（A-08）は第53セッションで 3 → 4 へ上げた。**
   *   ハザードの置き場所の判定を1つにそろえた（table.js の outR）ことで
   *   **L字の6番ホールだけ砂の位置が変わり**、上級者の打数が 3 → 4 になったため。
   *   種を3つ変えても 3・1・3・2・4・(4)・2・6 で同じ（上級者は狙いにぶれが無い）。
   *   **コースを触ったら、パーは測り直す。**パーは上級者の打数そのものだからである。
   * ★**星型（A-11）は第62セッションで 5 → 3 へ下げた。**池を半分にしたので
   *   上級者の打数が 6 → 3 になった（同じ理由＝コースを触ったから測り直した）。
   */
