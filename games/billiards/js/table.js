...
const BilliardsTable = (() => {
  ...
  function make(shape, hasPockets) {
    ...
    /*
     * ブレイクの既定の置き場所。**持たない台は null** で、画面側が
     * ヘッドスポットの少しヘッド側（160 mm）を使う（10.2.6節）。
     * breakSide を持つ台は、手玉を置ける線の上の「玉が収まるいちばん端」へ寄せる。
   ...
   */
  /*
   * 目標円の面積が盤面に占める割合（D404で確定。7.9.2節）。
  ...
  function playArea(table) {
    const o = Math.abs(polyCentroid(table.outline).area);
    const i = table.innerOutline ? Math.abs(polyCentroid(table.innerOutline).area) : 0;
   ...
   *   形が変わっても同じ1本の規定で済み、短すぎと行き過ぎの両方を同時に受けられる。
   */
