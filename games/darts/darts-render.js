...
// センサー軸 → 視線方向の符号（実機テストで決定: v1.02 で逆だったため全反転）
// window-into-3D 物理: 端末を右に振ったら世界（壁+的）は左に動く
const SIGN_YAW = -1;    // rel.alpha → yawDelta
const SIGN_PITCH = +1;  // rel.beta → pitchDelta
const SIGN_ROLL = -1;   // rel.gamma → roll (rotateZ) — v1.04 で再反転
const ROLL_SCALE = 0.5; // ロール感度（v1.05 で 1.0 → 0.5、ユーザー指示）
...
function tick() {
  ...
  if (rel) {
    ...
    roll       = SIGN_ROLL  * (rel.gamma || 0) * ROLL_SCALE;
  }
