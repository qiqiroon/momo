...
// センサー軸 → 視線方向の符号（端末/ブラウザにより要調整。debug 表示で確認）
const SIGN_YAW = +1;    // rel.alpha → yawDelta
const SIGN_PITCH = -1;  // rel.beta → pitchDelta（beta が減ると上向き）
const SIGN_ROLL = -1;   // rel.gamma → roll (rotateZ)
...
// ======================================================================
// rAF ループ：センサー値 → 板の transform 更新 + 矢印切替
// ======================================================================
function tick() {
  if (!_boardEl || !_viewEl) {
    _animFrameId = null;
  ...
  if (rel) {
    yawDelta   = SIGN_YAW   * (rel.alpha || 0);
    pitchDelta = SIGN_PITCH * (rel.beta  || 0);
    roll       = SIGN_ROLL  * (rel.gamma || 0);
  }
  ...
  // 中心からのオフセット分だけ板が傾いて見える（なんちゃって 3D）
  ...
  _boardEl.style.transform =
    `translate(-50%, -50%) translate3d(${x.toFixed(1)}px, ${y.toFixed(1)}px, 0px) ` +
...
// 起動・停止
// ======================================================================
export function start({ viewEl, boardEl, arrowEl, debugCallback }) {
  _viewEl = viewEl;
  _boardEl = boardEl;
  _arrowEl = arrowEl;
