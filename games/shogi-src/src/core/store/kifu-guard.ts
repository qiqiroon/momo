 ...
 *
 * **★指しかけの対局も、ここで棋譜として残せるようにする**（v1.43・2026-08-17 ユーザー報告）。
 * 棋譜が生まれるのは終局の瞬間だけだったので、**対局中にリセットすると、その対局は
 * 保存する機会が一度も無いまま消えていた**。リセットは対局をそこで打ち切る操作なので、
 * 打ち切る直前の盤から棋譜を作り、二段目で「保存する／破棄する／やめる」を尋ねる。
 *
 * 作るのは**一段目に「はい」と答えたあと**＝リセットする意思が示された時点。
 * 押しただけでは作らない（**やめる人の記憶を勝手に書き換えない**）。
 */
export function guardResetYes(): void {
  ...
  pluginGet<() => void>('kifu:captureCurrent')?.();
  askKifuOrRun(action);
}
