  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      if (window.innerWidth !== seenW || window.innerHeight !== seenH) {
        ...
        applied = 0;
        // ★v1.63: **窓の大きさが変わったら、まず画面の左上へ戻す**
        // （2026-08-20 ユーザーご依頼）。
        //
        // **なぜ要るのか**＝横長は「縦にも横にもスクロールさせない」前提なので、
        // ふだんは左上にいる。ところが**何かの理由で下へスクロールしていると、
        // 盤の上側が窓の外へ出て切れる**。切れたぶんは見えないので、**盤が小さくなり、
        // 下に空間が広がったように見える**（実測＝デバッグ表示を出すと右の列が 727px に
        // 伸びてページがスクロールできるようになり、下げた状態では盤の上が隠れる）。
        // 大きさの計算そのものはスクロール位置に左右されない（高さしか見ていないため・
        // 実測＝下までスクロールして引きずっても上に戻して引きずっても盤 416px で同じ）。
        // **直しているのは見え方のほう。**
        //
        // **縦長では戻さない**＝縦長は下へあふれるのが規定で、**下の方（チャット等）を
        // 見ている最中に窓を変えると、読んでいた場所から引き剥がされる**ため。
        if (window.innerWidth >= window.innerHeight && (window.scrollX || window.scrollY)) {
          window.scrollTo(0, 0);
        }
