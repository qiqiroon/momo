...
export function ReviewScreen() {
  ...
  const onPicked = async (input: HTMLInputElement) => {
    ...
    try {
      ...
      // ★段2: **差し替える前にカスタムルールの定義を取り戻す**（§9.2.6）。公式・手元で
      // 取り戻せなければファイル選択のパネルを出し、選べてから差し替える。
  ...
  useEffect(() => {
    if (file) {
      ...
      //
      // v1.66 まではここで引き返していたので、**棋譜を持たない部屋では、届いた手を
      // 載せる下の処理までたどり着かなかった**＝相手が盤を組み替えても自分の盤が
      // 動かない（印は盤を組み立て直さないので、印だけが届いて見えていた）。
      //
      // **ルールは今の盤のまま**（引数を渡さない＝前回の値を引き継ぐ）。
      // **これは棋譜があるときに毎回通っている扉と同じ**＝`replayKifu` の中身も
      // 同じ作り直しであり、**新しい危険は増えない**。記憶が捨てられるのは
      // 「名乗っていない」かつ「印が保存済みか破棄予定」のときだけで、
      // **この画面は入っている間ずっと名乗り続けている**（§9.4.3・`holdReplayGuard`）。
    ...
    >
      ...
      <div className="grid">
        ...
        <div className="playbar">
          ...
          {/* ★v1.59: **掴んでいないときは何も出さない**（付録D-12 v1.8 §14.1・
              2026-08-20 実機のご報告）＝v1.58 までの「どちらの駒も自由に動かせます
              （時計は動きません）」は廃止した。**配っている最中・移っている最中の
              知らせはそのまま出す**（押せない理由は言葉で出す）。

              **★行は畳まない**＝空でも場所を空けておく。畳むと、駒を掴むたびに
              一言が現れて**盤が縦に動く**（出たり消えたりするものを詰めない）。 */}
