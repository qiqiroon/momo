...
export interface RoomConfig {
   ...
   *  部屋名・棋譜・準備完了カードに出す。**画面に出すためのもので、正体ではない**。 */
  customRuleName?: string;
  /**
   * ★段B②: **公式一覧 (`rules/`) から選んだときの目印**（`metadata.game_id`）。
   *
   * **入っている＝公式一覧のルール**なので、ネットでは定義を送らず、受け取った側が
   * この目印で自分で取ってくる（ユーザー判断 2026-08-25）。**自分のファイルから
   * 読み込んだルールには入らない**＝取りに行く先が無いので、ホストが定義を配る。
   */
  customRuleId?: string;
  /**
   * 読み込んだカスタムルールの定義そのもの（S02 で選んだもの）。**custom は定義が正体**
   * なので、種類の名札だけでは盤が作れない。**★段B②: この欄はネットへも送る**
   * （詰める／取り出すは `rulesSync` の 2 つだけを通る）。
   */
  customMgf?: Mgf;
