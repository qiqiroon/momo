...
CLIENT_ID   = "1053350886212-q87r5msugnqbb3saoq1fh3uj3t648hcg.apps.googleusercontent.com"
FIELDS_FILE = (
...
class MomoGDrive:
    ...
    def __init__(self, cache_ttl: int = CACHE_TTL) -> None:
        ...
        self._token_error: str | None = None  # OAuth2 エラー文字列

    # ---------- 認証 ----------

    async def connect(self, hint: str | None = None, prompt: str | None = None) -> None:
        """
        ...
        ----------
        hint   : str | None
            既知の Google アカウントのメールアドレス。
            指定するとサイレント再認証（prompt='none'）を試みる。
        prompt : str | None
            GIS の prompt パラメータ（'none' / 'consent' / 'select_account'）。
            省略時は hint がある場合に 'none'、ない場合はデフォルト動作。
        """
        js = _js()
        js.window.MOMO_GOOGLE_CLIENT_ID = CLIENT_ID
        ...
        if hint:
            config_pairs.append(["hint", hint])
        # prompt は initTokenClient の設定に渡す（requestAccessToken のオーバーライドより確実）
        if prompt:
            config_pairs.append(["prompt", prompt])

        token_client = js.google.accounts.oauth2.initTokenClient(
            js.Object.fromEntries(js.Map.new(config_pairs))
        )

        token_client.requestAccessToken()

        # _on_token で self._token / self._token_error がセットされるまで待つ
        # サイレント認証(prompt=none)は数秒で完了するはずなので短め、通常は60秒
        timeout = 8000 if prompt == "none" else 60000
        elapsed = 0
        while self._token is None and self._token_error is None:
            await _sleep_ms(100)
            elapsed += 100
            if elapsed >= timeout:
                raise TimeoutError(
                    "サイレント認証タイムアウト" if prompt == "none"
                    else "Google OAuth2 タイムアウト（60秒）"
                )

        if self._token_error:
            raise PermissionError(f"Google OAuth2 エラー: {self._token_error}")

    def _on_token(self, token_response: Any) -> None:
        """OAuth2 コールバック（js から呼び出される）。"""
        err = getattr(token_response, "error", None)
        if err:
            self._token_error = str(err)
        else:
