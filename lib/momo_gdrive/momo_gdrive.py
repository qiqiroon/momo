...
class MomoGDrive:
    ...
    def __init__(self, cache_ttl: int = CACHE_TTL) -> None:
        ...
        self._token_expires_at: float = 0            # UNIX timestamp（有効期限）

    # ---------- 認証 ----------

    async def connect(self, hint: str | None = None) -> None:
        """
        ...
        hint : str | None
            ...
            指定するとアカウント選択画面をスキップし、
            許可ボタンのみの画面を表示する（2回目以降の認証に使用）。
        """
        js = _js()
        ...
        # hint を渡すとアカウント選択をスキップして許可ボタンのみ表示
        if hint:
        ...
        while self._token is None and self._token_error is None:
            ...
            elapsed += 100
            if elapsed >= timeout:
                raise TimeoutError("Google OAuth2 タイムアウト（60秒）")
    ...
    def _on_token(self, token_response: Any) -> None:
        ...
        if err:
            ...
            self._token_error = None
            # expires_in（秒）から有効期限を計算（余裕を持って60秒引く）
            try:
                exp = int(getattr(token_response, "expires_in", 3600))
            except Exception:
                exp = 3600
            self._token_expires_at = time.time() + exp - 60

    def is_token_valid(self) -> bool:
        """保存済みトークンが有効期限内かどうかを返す。"""
