...
"""
MOMO Project: GitHub 共通ライブラリ (momo_github) v1.00
    ...
    返り値:
        {
            ...
            "reset_at":  datetime | None,  # リセット時刻（UTC）
...
class MomoGitHub:
    ...
    async def list_dir(
        ...
        if status in (403, 429):
            ...
            reset_str = (
                reset_at.strftime("%Y-%m-%d %H:%M:%S UTC") if reset_at else "unknown"
            )
            # レート制限時はキャッシュを優先返却
            cached = self._get_cache(url, ttl=None)
            if cached is not None:
