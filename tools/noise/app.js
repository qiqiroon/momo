...
async function saveToDrive() {
  ...
  try {
    ...
    // 全バッファをWAV保存（録音・ファイル読み込み両方）
    for (const btn of buttons) {
      if (!btn.buffer) continue;
...
async function loadFromDrive() {
  ...
  try {
    ...
    // WAVバッファ復元（hasBuffer=trueのボタンのWAVをDriveからダウンロード）
    if (data.meta) {
      for (const m of data.meta) {
...
async function previewButton(btnId) {
  ...
  const btn = buttons[btnId];
  if (!btn.buffer) return;
  // iOS: resume inside user gesture
  await resumeAudio();
  AudioEngine.init();
...
async function setMode(m) {
  mode = m;
  // iOS: resume AudioContext inside user gesture on every mode switch
  await resumeAudio();
