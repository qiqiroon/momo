// app.js - MOMO Noise Main Controller v0.11
'use strict';
...
// ── Play ──────────────────────────────────────────────────────────────────
// Track last scheduled beat time per button to prevent double-play
const lastBeatTime = new Map();
...
async function triggerPlay(btnId) {
  ...
  // Sync mode: block if same beat time already scheduled for this button
  if (metroOn && syncOn) {
    const last = lastBeatTime.get(btnId);
    if (last !== undefined && Math.abs(startAt - last) < 0.01) return;
    lastBeatTime.set(btnId, startAt);
  }
...
async function startRecord(btnId) {
  ...
  try {
    // startRecording now resolves after onstart (recording actually begun)
    await AudioEngine.startRecording();
    recBtnId = btnId;
    // Show recording status in audio label, switch to stop button ASAP
    renderEditRow(btnId);
  } catch(e) {
...
function renderEditRow(btnId) {
  ...
  // Buttons: rec/stop switch — no persistent color on either button
  const rb = row.querySelector('.rec-btn');
  const sb = row.querySelector('.stop-btn');
  rb.textContent = t('record');
  sb.textContent = t('record'); // stop btn reverts to rec label after stop
  rb.style.display = isRec ? 'none' : '';
  sb.style.display = isRec ? '' : 'none';
  // Clear any lingering highlight classes from both buttons
  rb.classList.remove('recording', 'active');
  sb.classList.remove('active', 'btn-danger'); // no persistent red on stop btn
  // Stop btn: orange only while recording is active (via rec-active on status label)
  if (isRec) sb.classList.add('btn-rec-active');
  else sb.classList.remove('btn-rec-active');
  ...
  buttons.forEach((btn, idx) => {
    ...
    row.innerHTML = `
      ...
      <div class="btn-actions">
        ...
        <button class="stop-btn noise-btn" style="display:none"></button>
    ...
    // Rec button: momentary orange on press, start recording on release
    const recBtn  = row.querySelector('.rec-btn');
    const stopBtn = row.querySelector('.stop-btn');
    recBtn.addEventListener('pointerdown', () => recBtn.classList.add('active'));
    recBtn.addEventListener('pointerup',   () => { recBtn.classList.remove('active'); startRecord(idx); });
    recBtn.addEventListener('pointerleave',() => recBtn.classList.remove('active'));
    // Stop button: momentary orange on press, stop on release, revert to rec display
    stopBtn.addEventListener('pointerdown', () => stopBtn.classList.add('active'));
    stopBtn.addEventListener('pointerup',   () => { stopBtn.classList.remove('active'); stopRecord(idx); });
    stopBtn.addEventListener('pointerleave',() => stopBtn.classList.remove('active'));
...
async function init() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
      ...
      // Screen going dark: clear active state (onEnded won't fire after resume)
