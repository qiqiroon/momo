...
// iOS: wait until AudioContext is confirmed running (up to 500ms)
async function waitAudioRunning() {
  ...
  await new Promise(resolve => {
    const check = () => {
      if (actx.state === 'running') { resolve(); return; }
      actx.resume().catch(() => {}).finally(() => setTimeout(check, 50));
    };
    setTimeout(check, 20);
...
async function saveToDrive() {
  ...
  showToast('0 / ' + targets.length + ' 保存中…', 60000);
  try {
    // 設定JSON保存
