// app.js - MOMO Noise Main Controller v0.13
'use strict';
...
async function triggerPlay(btnId) {
  ...
  // Sync: block duplicate play scheduled at same beat (within 50ms window)
  if (metroOn && syncOn) {
    const last = lastScheduledAt.get(btnId) ?? -1;
    if (startAt - last < 0.05) {
      dbg('double-play blocked', { btnId, startAt: startAt.toFixed(3), last: last.toFixed(3) });
      return;
    }
    lastScheduledAt.set(btnId, startAt);
    dbg('play scheduled', { btnId, startAt: startAt.toFixed(3), delay: (startAt-now).toFixed(3) });
  }
...
async function startRecord(btnId) {
  ...
  try {
    ...
    if (row) {
      ...
      dbgClassList(row.querySelector('.stop-btn'), 'stop-btn after start');
      dbg('rec display', row.querySelector('.rec-btn').style.display);
      dbg('stop display', row.querySelector('.stop-btn').style.display);
    }
  } catch(e) {
...
async function stopRecord(btnId) {
  ...
  const row = document.querySelector(`[data-edit-row="${btnId}"]`);
  if (row) {
    dbgClassList(row.querySelector('.rec-btn'),  'rec-btn after stop');
    dbgClassList(row.querySelector('.stop-btn'), 'stop-btn after stop');
  }
  saveToDb();
}
...
function renderEditRow(btnId) {
  ...
  // Buttons: rec/stop switch
  const rb = row.querySelector('.rec-btn');
  const sb = row.querySelector('.stop-btn');
  rb.textContent = t('record');
  sb.textContent = t('recording');  // shows stop label while recording
  // Strip ALL state classes before applying new state
  rb.className = 'rec-btn noise-btn';
  sb.className = 'stop-btn noise-btn' + (isRec ? ' btn-rec-active' : '');
