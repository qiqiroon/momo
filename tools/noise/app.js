// app.js - MOMO Noise Main Controller v0.09
'use strict';
...
function renderEditRow(btnId) {
  ...
  // Buttons: rec/stop switch — no persistent color on rec button
  const rb = row.querySelector('.rec-btn');
  const sb = row.querySelector('.stop-btn');
  rb.textContent = t('record');
  sb.textContent = t('record'); // stop btn shows '録音' label (back to record state visually)
  rb.style.display = isRec ? 'none' : '';
  sb.style.display = isRec ? '' : 'none';
  rb.classList.remove('recording'); // never persistent orange on rec btn
