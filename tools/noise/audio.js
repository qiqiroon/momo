...
const AudioEngine = (() => {
  ...
  async function startRecording() {
    ...
    // Debug: collect environment info for toast
    const debugInfo = [
      'UA:' + navigator.userAgent.slice(0, 60),
      'MR:' + (typeof MediaRecorder !== 'undefined' ? 'yes' : 'NO'),
      'gUM:' + (navigator.mediaDevices?.getUserMedia ? 'yes' : 'NO'),
      'ctx:' + ctx.state,
    ];
    if (window._debugToast) window._debugToast(debugInfo.join(' | '));
    ...
    try {
      ...
      if (window._debugToast) window._debugToast('gUM ERR: ' + e.name + ': ' + e.message);
      throw e;
    }
    recordingChunks = [];
    ...
    if (window._debugToast) window._debugToast('mime:' + (mimeType || 'default'));
    ...
    try {
      ...
      if (window._debugToast) window._debugToast('MR ERR: ' + e.name + ': ' + e.message);
