// drive.js - MOMO Noise Google Drive Integration v0.02
'use strict';
...
const Drive = (() => {
  ...
  async function uploadWav(name, wavBlob) {
    ...
    let resp;
    if (existingId) {
      // 既存ファイル更新: media uploadでバイナリのみ送信
      resp = await fetch(`${UPLOAD_BASE}/files/${existingId}?uploadType=media`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
      ...
      const meta = { name, parents: [folderId], mimeType: 'audio/wav' };
      const form = new FormData();
      form.append('metadata', new Blob([JSON.stringify(meta)], { type: 'application/json' }));
      form.append('file', wavBlob, name);
      resp = await fetch(`${UPLOAD_BASE}/files?uploadType=multipart`, {
        method: 'POST',
  ...
  // WAVファイルをDriveからダウンロードしてArrayBufferで返す
  async function downloadWav(name) {
    if (!folderId) throw new Error('Not connected');
    const q = `name='${name}' and '${folderId}' in parents and trashed=false`;
    const data = await apiFetch(`${API_BASE}/files?q=${encodeURIComponent(q)}&fields=files(id)`);
    const fileId = data.files[0]?.id;
  ...
  return { isSignedIn, signIn, signInSilent, signOut, listFiles, uploadWav, saveJson, loadJson, downloadWav };
})();
